namespace AgroDictionary.Forms
{
    partial class InfoForm
    {
        /// <summary>
        /// Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        /// Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        /// Required method for Designer support - do not modify
        /// the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            exp_date_label = new Label();
            hybridity_of_culture_label = new Label();
            fetal_weight_label = new Label();
            matur_time_label = new Label();
            label8 = new Label();
            label17 = new Label();
            label7 = new Label();
            label9 = new Label();
            description_of_culture_textbox = new TextBox();
            type_of_culture_label = new Label();
            name_of_culture_label = new Label();
            label4 = new Label();
            label3 = new Label();
            label2 = new Label();
            label1 = new Label();
            menuStrip1 = new MenuStrip();
            редагуватиРослинуToolStripMenuItem = new ToolStripMenuItem();
            menuStrip1.SuspendLayout();
            SuspendLayout();
            // 
            // exp_date_label
            // 
            exp_date_label.AutoSize = true;
            exp_date_label.Font = new Font("Sitka Banner", 10.8F, FontStyle.Bold);
            exp_date_label.ForeColor = Color.Snow;
            exp_date_label.Location = new Point(166, 291);
            exp_date_label.Name = "exp_date_label";
            exp_date_label.Size = new Size(16, 26);
            exp_date_label.TabIndex = 38;
            exp_date_label.Text = " ";
            // 
            // hybridity_of_culture_label
            // 
            hybridity_of_culture_label.AutoSize = true;
            hybridity_of_culture_label.Font = new Font("Sitka Banner", 10.8F, FontStyle.Bold);
            hybridity_of_culture_label.ForeColor = Color.Snow;
            hybridity_of_culture_label.Location = new Point(166, 251);
            hybridity_of_culture_label.Name = "hybridity_of_culture_label";
            hybridity_of_culture_label.Size = new Size(16, 26);
            hybridity_of_culture_label.TabIndex = 37;
            hybridity_of_culture_label.Text = " ";
            // 
            // fetal_weight_label
            // 
            fetal_weight_label.AutoSize = true;
            fetal_weight_label.Font = new Font("Sitka Banner", 10.8F, FontStyle.Bold);
            fetal_weight_label.ForeColor = Color.Snow;
            fetal_weight_label.Location = new Point(166, 214);
            fetal_weight_label.Name = "fetal_weight_label";
            fetal_weight_label.Size = new Size(16, 26);
            fetal_weight_label.TabIndex = 36;
            fetal_weight_label.Text = " ";
            // 
            // matur_time_label
            // 
            matur_time_label.AutoSize = true;
            matur_time_label.Font = new Font("Sitka Banner", 10.8F, FontStyle.Bold);
            matur_time_label.ForeColor = Color.Snow;
            matur_time_label.Location = new Point(166, 174);
            matur_time_label.Name = "matur_time_label";
            matur_time_label.Size = new Size(16, 26);
            matur_time_label.TabIndex = 35;
            matur_time_label.Text = " ";
            // 
            // label8
            // 
            label8.AutoSize = true;
            label8.Font = new Font("Sitka Banner", 10.8F, FontStyle.Bold, GraphicsUnit.Point, 0);
            label8.ForeColor = Color.Snow;
            label8.Location = new Point(58, 251);
            label8.Name = "label8";
            label8.Size = new Size(102, 26);
            label8.TabIndex = 31;
            label8.Text = "Гібридність";
            // 
            // label17
            // 
            label17.AutoSize = true;
            label17.Font = new Font("Sitka Banner", 10.8F, FontStyle.Bold, GraphicsUnit.Point, 0);
            label17.ForeColor = Color.Snow;
            label17.Location = new Point(8, 291);
            label17.Name = "label17";
            label17.Size = new Size(152, 26);
            label17.TabIndex = 34;
            label17.Text = "Термін зберігання";
            // 
            // label7
            // 
            label7.AutoSize = true;
            label7.Font = new Font("Sitka Banner", 10.8F, FontStyle.Bold, GraphicsUnit.Point, 0);
            label7.ForeColor = Color.Snow;
            label7.Location = new Point(57, 214);
            label7.Name = "label7";
            label7.Size = new Size(96, 26);
            label7.TabIndex = 33;
            label7.Text = "Вага плоду";
            // 
            // label9
            // 
            label9.AutoSize = true;
            label9.Font = new Font("Sitka Banner", 10.8F, FontStyle.Bold, GraphicsUnit.Point, 0);
            label9.ForeColor = Color.Snow;
            label9.Location = new Point(30, 174);
            label9.Name = "label9";
            label9.Size = new Size(130, 26);
            label9.TabIndex = 32;
            label9.Text = "Час дозрівання";
            // 
            // description_of_culture_textbox
            // 
            description_of_culture_textbox.Location = new Point(170, 333);
            description_of_culture_textbox.Name = "description_of_culture_textbox";
            description_of_culture_textbox.Size = new Size(300, 27);
            description_of_culture_textbox.TabIndex = 30;
            // 
            // type_of_culture_label
            // 
            type_of_culture_label.AutoSize = true;
            type_of_culture_label.Font = new Font("Sitka Banner", 10.8F, FontStyle.Bold);
            type_of_culture_label.ForeColor = Color.Snow;
            type_of_culture_label.Location = new Point(166, 134);
            type_of_culture_label.Name = "type_of_culture_label";
            type_of_culture_label.Size = new Size(16, 26);
            type_of_culture_label.TabIndex = 29;
            type_of_culture_label.Text = " ";
            // 
            // name_of_culture_label
            // 
            name_of_culture_label.AutoSize = true;
            name_of_culture_label.Font = new Font("Sitka Banner", 10.8F, FontStyle.Bold);
            name_of_culture_label.ForeColor = Color.Snow;
            name_of_culture_label.Location = new Point(166, 97);
            name_of_culture_label.Name = "name_of_culture_label";
            name_of_culture_label.Size = new Size(16, 26);
            name_of_culture_label.TabIndex = 28;
            name_of_culture_label.Text = " ";
            // 
            // label4
            // 
            label4.AutoSize = true;
            label4.Font = new Font("Sitka Banner", 10.8F, FontStyle.Bold);
            label4.ForeColor = Color.Snow;
            label4.Location = new Point(103, 331);
            label4.Name = "label4";
            label4.Size = new Size(50, 26);
            label4.TabIndex = 27;
            label4.Text = "Опис";
            // 
            // label3
            // 
            label3.AutoSize = true;
            label3.Font = new Font("Sitka Banner", 10.8F, FontStyle.Bold);
            label3.ForeColor = Color.Snow;
            label3.Location = new Point(38, 134);
            label3.Name = "label3";
            label3.Size = new Size(115, 26);
            label3.TabIndex = 26;
            label3.Text = "Вид культури";
            // 
            // label2
            // 
            label2.AutoSize = true;
            label2.Font = new Font("Sitka Banner", 10.8F, FontStyle.Bold);
            label2.ForeColor = Color.Snow;
            label2.Location = new Point(23, 97);
            label2.Name = "label2";
            label2.Size = new Size(130, 26);
            label2.TabIndex = 25;
            label2.Text = "Назва культури";
            // 
            // label1
            // 
            label1.AutoSize = true;
            label1.Font = new Font("Sitka Banner", 16.1999989F, FontStyle.Bold, GraphicsUnit.Point, 0);
            label1.ForeColor = Color.Snow;
            label1.Location = new Point(160, 39);
            label1.Name = "label1";
            label1.Size = new Size(299, 39);
            label1.TabIndex = 24;
            label1.Text = "Інформація про рослину";
            // 
            // menuStrip1
            // 
            menuStrip1.BackColor = Color.DarkOrange;
            menuStrip1.Font = new Font("Times New Roman", 10.8F, FontStyle.Bold, GraphicsUnit.Point, 0);
            menuStrip1.ImageScalingSize = new Size(20, 20);
            menuStrip1.Items.AddRange(new ToolStripItem[] { редагуватиРослинуToolStripMenuItem });
            menuStrip1.Location = new Point(0, 0);
            menuStrip1.Name = "menuStrip1";
            menuStrip1.Size = new Size(723, 28);
            menuStrip1.TabIndex = 39;
            menuStrip1.Text = "menuStrip1";
            // 
            // редагуватиРослинуToolStripMenuItem
            // 
            редагуватиРослинуToolStripMenuItem.Name = "редагуватиРослинуToolStripMenuItem";
            редагуватиРослинуToolStripMenuItem.Size = new Size(187, 24);
            редагуватиРослинуToolStripMenuItem.Text = "Редагувати рослину";
            // 
            // InfoForm
            // 
            AutoScaleDimensions = new SizeF(8F, 20F);
            AutoScaleMode = AutoScaleMode.Font;
            BackColor = Color.ForestGreen;
            ClientSize = new Size(723, 380);
            Controls.Add(exp_date_label);
            Controls.Add(hybridity_of_culture_label);
            Controls.Add(fetal_weight_label);
            Controls.Add(matur_time_label);
            Controls.Add(label8);
            Controls.Add(label17);
            Controls.Add(label7);
            Controls.Add(label9);
            Controls.Add(description_of_culture_textbox);
            Controls.Add(type_of_culture_label);
            Controls.Add(name_of_culture_label);
            Controls.Add(label4);
            Controls.Add(label3);
            Controls.Add(label2);
            Controls.Add(label1);
            Controls.Add(menuStrip1);
            MainMenuStrip = menuStrip1;
            Name = "InfoForm";
            Text = "InfoForm";
            menuStrip1.ResumeLayout(false);
            menuStrip1.PerformLayout();
            ResumeLayout(false);
            PerformLayout();
        }

        #endregion
        private Label exp_date_label;
        private Label hybridity_of_culture_label;
        private Label fetal_weight_label;
        private Label matur_time_label;
        private Label label8;
        private Label label17;
        private Label label7;
        private Label label9;
        private TextBox description_of_culture_textbox;
        private Label type_of_culture_label;
        private Label name_of_culture_label;
        private Label label4;
        private Label label3;
        private Label label2;
        private Label label1;
        private MenuStrip menuStrip1;
        private ToolStripMenuItem редагуватиРослинуToolStripMenuItem;
    }
}